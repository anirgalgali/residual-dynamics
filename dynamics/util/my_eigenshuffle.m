function [Vseq,Vseq_r,Dseq] = my_eigenshuffle(Asequence,sort_type)
...
for i = 1:n
  ...
  switch sort_type
      ...
      case 'abs'
           ...
           [junk,tags] = sort(abs(D),1,'descend');
  end
  
  Dseq(:,i) = D(tags);
  Vseq(:,:,i) = V(:,tags);
  Vseq_r(:,:,i) = Vseq(:,:,i);
  idx = find(imag(Dseq(:,i)) ~= 0);
  
  if(~isempty(idx))
      
      num_conj = length(idx)/2;
      
      for iconj = 1:num_conj
          
          idx_pair = idx((iconj - 1)*2 + 1 : (iconj - 1)*2 + 2);
          Vseq_r(:,idx_pair,i) = getRealVs(Vseq(:,idx_pair,i) , Dseq(idx_pair,i));
...
for i = 2:n
  ...
  Vseq_r(:,:,i) = Vseq_r(:,reorder,i);
  Dseq(:,i) = Dseq(reorder,i);
  
  % also ensure the signs of each eigenvector pair
  % were consistent if possible
  S = squeeze(real(sum(Vseq_r(:,:,i-1).*Vseq_r(:,:,i),1))) < 0;
  Vseq_r(:,S,i) = -Vseq_r(:,S,i);
  S = squeeze(real(sum(Vseq(:,:,i-1).*Vseq(:,:,i),1))) < 0;
  Vseq(:,S,i) = -Vseq(:,S,i);
%   idx_imag = imag(Dseq(:,i) ~= 0);
%   idx_real = imag(Dseq(:,i)) == 0;
%   Vseq(:,idx_real & S,i) = -Vseq_r(:,idx_real & S,i);
%   if(~isempty(idx))
%       
%       num_conj = length(idx)/2;
%       
...
%           
%           idx_pair = idx((iconj - 1)*2 + 1 : (iconj - 1)*2 + 2);
%           Vseq_r(:,S(idx_pair),i) = -Vseq(:,S(idx_pair),i);
%           
%       end
%       
%       
