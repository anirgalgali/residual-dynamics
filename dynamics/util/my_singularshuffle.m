function [Useq, Sseq, Vseq] = my_singularshuffle(Asequence,align_type)
...
for i = 2:n
  ...
  Useq(:,:,i) = Useq(:,reorder,i);
  Vseq(:,:,i) = Vseq(:,reorder,i);
  Sseq(:,i) = Sseq(reorder,i);

  F = squeeze(real(sum(Vseq(:,:,i-1).*Vseq(:,:,i),1))) < 0;
  Vseq(:,F,i) = -Vseq(:,F,i);
  
  
  F = squeeze(real(sum(Useq(:,:,i-1).*Useq(:,:,i),1))) < 0;
 Useq(:,F,i) = -Useq(:,F,i);
