%{ This script simulates an augmented rotational dynamics model, which
...
rseed =  model_analyses.simPars.model1.model_obj.RAND_SEED; % Taken from Toy DynamicsModel.m 
rng(rseed);
...
n_trials = model_analyses.simPars.model1.model_obj.num_trajectories;
n_times = model_analyses.simPars.model1.model_obj.sim_time;
dt = model_analyses.simPars.model1.model_obj.TIME_STEP;

% parameters determining inital condition of latent states
x_latent_noise_var = unique(diag(model_analyses.simPars.model1.model_obj.noise_var));

% parameters determining choice-dependent input time-course
% choice_dep_input_magnitude = 8;
% choice_dep_input_direction = [-1;1];
choice_dep_input_magnitude = [];
...
choice_labels = model_analyses.data.states.model1.choice_labels;
model_optimized_inputs = model_analyses.data.inputs.model1.mean;
...
% choice_dep_input_magnitude_t = repmat(choice_dep_input_magnitude.*ones(n_choice_dep_dims,1),[1 n_times]);
...
%Determining varaince of initial conditions for condition-independent
%inputs
...
[v_eig_la, d_eig_la] = eig(pars_rotational.A);
...
x0_original_dims = squeeze(model_analyses.data.states.model1.response(:, 1 , :));
...
% Generating observations and binned residuals
...
line_length_modulator = 4;
base_line_length = 0.02;
...
for ialign = 1: length(alignments)
    ...
    for ii = 1: length(analysis_condavg.(alignments{ialign}).projections)
        ...
        if(strcmp(analysis_condavg.(alignments{ialign}).direction_labels{ii},'jPC34') | strcmp(analysis_condavg.(alignments{ialign}).direction_labels{ii},'choice'))
            flow_pars.LineLength = line_length_modulator * base_line_length;
        else
            flow_pars.LineLength = base_line_length;
        end
...
if(do_save_fig)
    export_fig(fullfile(save_fig_path, sprintf('%s%d%s%.1f%s%.1f','condavg2Dtaskplanes_augmentedrotational_planeangle=',...
        angle_between_planes,'_lambda_decay=',lambda_decay_1,'_',lambda_decay_2)),'-painters','-transparent','-pdf')
end
...
h = plotCV_lagDim_v2(analysis_resdyn.cv_lagdim, analysis_resdyn.cv_lagdim_hyp, plotpars);
ax = get(h,'Children');
ax = flipud(ax);
for iax = 1: length(ax)
...
if(do_save_fig)
    export_fig(fullfile(save_fig_path, sprintf('%s%d%s%.1f%s%.1f','ResDynFits_augmentedrotational_planeangle=',...
    angle_between_planes,'_lambda_decay=',lambda_decay_1,'_',lambda_decay_2)),'-painters','-transparent','-pdf')
end
...
%%
if(do_orth_obs_mat)
    result.data_identifier = sprintf('%s%s%d%s%d','augmentedrotational','_orthC=',double(do_orth_obs_mat),...
        '_anglebwplanes=',angle_between_planes);
else
    result.data_identifier = sprintf('%s%s%d','augmentedrotational','_orthC=',double(do_orth_obs_mat));
end
